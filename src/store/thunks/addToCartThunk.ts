...
export const addToCartThunk = createAsyncThunk<AddToCartPayload, AddToCartPayload, AsyncThunkConfig>(
  "addToCart",
  async ({ product, quantity }, { rejectWithValue, getState }) => {
    const cart = getState().cart;

    if (cart.products && cart.products[product.id] && cart.products[product.id].quantity + quantity > MAX_PRODUCTS_OF_SAME_TYPE) {
